...
_CONFIG_FNAME = 'wdsac_network_config.json'
...
def network_config(
    ...
    network_factory: types.NetworkFactory[wdsac_networks.WDSACNetworks],
...
def _get_network(
  ...
  """Generates a WDSAC network given config."""
...
def load_config(
  ...
  """Loads WDSAC config from checkpoint."""
...
def load_policy(
  ...
  """Loads policy inference function from WDSAC checkpoint."""
  path = epath.Path(path)
  config = load_config(path)
  params = load(path)
  wdsac_network = _get_network(config, network_factory)
  make_inference_fn = wdsac_networks.make_inference_fn(wdsac_network)
